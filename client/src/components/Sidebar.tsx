    ...
    return (
          ...
          .map((message) => {
            ...
            return(
              <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                {!isUser && (
                  <div className={`w-8 h-8 rounnded-full bg-linear-to-br from-indigo-600 to-indigo-700 flex items-center justify-center`}>
                    <BotIcon className='size-5 text-white'/>
                  </div>
                )}

                <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ?  
                ...
                {isUser && (
                  <div className={`w-8 h-8 rounnded-full bg-gray-700  flex items-center justify-center`}>
        ...
        {isGenerating && (
            <div className='flex items-start gap-3 justify-start'>
                ...
                <span className='w-2 h-2 rounded-full animate-bounce bg-gray-600'
        ...
        <form onSubmit={handleRevisions} className='m-3 relative'>
            <div className='flex items-center gap-2'>
                ...
                className='flex-1 p-3  rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500
                bg-gray-800 text-gray-100 placeholder:text-gray-400 transition-all' disabled={isGenerating}/>
                <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full
                bg-linear-to-r from-indigo-500 hover:from-indigo-600
